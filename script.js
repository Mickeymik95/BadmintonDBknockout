...
function generateTeamList() {
    ...
    // Sort teams by seed number (1-16)
    ...
    sortedTeams.forEach(team => {
        ...
        // Skip BYE teams or show them differently

        if (team.nama === 'BYE' || !team.nama || team.nama === '...') {

            teamItem.style.opacity = '0.3';

            teamItem.style.border = '1px dashed #666';
        ...
        teamItem.innerHTML = `
            ...
            <div class="team-avatar" id="team_av_${team.seed}">

                ${getAvatarInitials(team.nama)}
        ...
        const avatarEl = document.getElementById(`team_av_${team.seed}`);

        if (avatarEl) {

            if (team.avatar && team.avatar.trim() !== '') {

                avatarEl.style.backgroundImage = `url('${team.avatar}')`;
                ...
                const color = getAvatarColor(team.nama);

                avatarEl.style.background = (team.nama === 'BYE' || !team.nama || team.nama === '...') 
